...
class BenchmarkVisualizer:
    def __init__(self, session_dir: str, output_dir: Optional[str] = None):
        ...
        self.output_dir = (
            Path(output_dir) if output_dir else self.session_dir / "graphs"
        )
    ...
    def _load_resource_data(self):
        ...
        try:
            ...
            if "benchmark" in resource_data.columns:
                resource_data["benchmark"] = resource_data["benchmark"].astype(
                    "category"
                )
        ...
        try:
            ...
            for i in range(header_idx + 1, len(lines)):
                ...
                if len(parts) >= 7:
                    ...
                    max_start = self._extract_number_with_prefix(
                        max_start_str, "max start:"
                    )
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if len(parts) < 6:
                            if self.debug_mode:
                                print(
                                    f"DEBUG: Skipping line with insufficient parts: {line}"
                                )
                            continue
                        ...
                        if task_match:
                            ...
                            if len(parts) >= 6:
                                ...
                                if self.debug_mode:
                                    print(
                                        f"DEBUG: Could not parse task info from line: {line}"
                                    )
                                continue
    ...
    def _extract_number_with_prefix(
    ...
    def _get_perf_metric_value(self, data, metric_name: str) -> Optional[float]:
        if isinstance(data, list):  # If data is a list of dicts
            ...
            try:
                metric_row = data[
                    data["event"].str.contains(metric_name, na=False, case=False)
                ]
    ...
    def plot_execution_times(self):
        ...
        for bench_name in self.hyperfine_paths:
            ...
            if "json" in data:
                ...
                if not df.empty and "mean" in df.columns:
                    ...
                    mins.append(
                        df["min"].iloc[0] if "min" in df.columns else df["mean"].iloc[0]
                    )
                    maxs.append(
                        df["max"].iloc[0] if "max" in df.columns else df["mean"].iloc[0]
                    )
        ...
        ax1.set_title(
            f"Mean Execution Times with Standard Deviation\n"
        ...
        if distributions and any(len(d) > 1 for d in distributions):
            ...
            if valid_distributions:
                bp = ax2.boxplot(
                    valid_distributions, labels=valid_labels, patch_artist=True
                )
                ...
                if use_log_scale:
                    ax2.set_yscale("log")
                    ax2.set_ylabel(
                        "Execution Time (seconds) - Log Scale", fontweight="bold"
                    )
        ...
        if mins and maxs:
            ...
            for i, (clean_name, mean_val, min_val, max_val) in enumerate(
                zip(clean_names, means, mins, maxs)
            ):
        ...
        if means:
            ...
            colors = [
                "green" if rp == 1.0 else "orange" if rp < 1.5 else "red"
            ...
            ax4.axhline(
                ...
                linestyle="--",
                alpha=0.7,
                label="Baseline (fastest)",
            )
            ax4.grid(True, alpha=0.3)
            ax4.legend(
                loc="upper left", bbox_to_anchor=(0, 1.15), frameon=True, fancybox=True
            )
        ...
        if distributions and any(len(d) > 5 for d in distributions):
            ...
            if valid_distributions:
                ...
                ax_violin.set_title(
                    "Execution Time Distribution - Violin Plot (Insufficient Data)"
                )
        else:
            ax_violin.text(
                0.5,
                0.5,
                "No distribution data available for violin plot",
                transform=ax_violin.transAxes,
                ha="center",
                va="center",
                fontsize=12,
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "execution_times.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
        print("✓ Generated execution_times.png")

        self._generate_execution_summary_table(
            benchmarks, clean_names, means, stds, medians, mins, maxs
        )
    ...
    def plot_performance_trends(self):
        ...
        if "datetime" in resource_data.columns:
            resource_data_sorted = resource_data.sort_values("datetime").reset_index(
                drop=True
            )
            ...
            if len(x) > 2:
                ...
                ax1.set_ylabel("Execution Time (seconds)", fontweight="bold")
                ax1.set_title(
                    "Performance Trend Analysis", fontweight="bold", fontsize=14
                )
            ...
            if len(resource_data_sorted) > 1:
                ...
                ax2.plot(
                    ...
                    "r-",
                    linewidth=2,
                    label=f"{window_size}-point Rolling Mean",
                )
                ax2.fill_between(
                    x,
                ...
                ax2.set_ylabel("Execution Time (seconds)", fontweight="bold")
                ax2.set_title(
                    "Performance Stability Analysis", fontweight="bold", fontsize=14
                )
                ax2.legend(loc="best", frameon=True, fancybox=True, shadow=True)
                ax2.grid(True, alpha=0.3)

            cpu_time = (
                resource_data_sorted["user_time"] + resource_data_sorted["system_time"]
            )
            ...
            sns.histplot(
                resource_data_sorted["elapsed_time"],
                bins=min(15, len(resource_data_sorted)),
                kde=True,
                ax=ax4,
                color="skyblue",
                edgecolor="black",
            ...
            for p, color in zip(percentiles, colors):
                ...
                ax4.axvline(
                    ...
                    linestyle="--",
                    alpha=0.8,
                    label=f"{p}th percentile: {percentile_val:.2f}s",
                )

            ax4.axvline(
                resource_data_sorted["elapsed_time"].mean(),
                color="blue",
                linestyle="-",
                linewidth=2,
                label=f"Mean: {resource_data_sorted['elapsed_time'].mean():.2f}s",
            )
            ax4.axvline(
                resource_data_sorted["elapsed_time"].median(),
                color="purple",
                linestyle="-.",
                linewidth=2,
                label=f"Median: {resource_data_sorted['elapsed_time'].median():.2f}s",
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "performance_trends.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
        print("✓ Generated performance_trends.png")

    def _generate_execution_summary_table(
        self, benchmarks, clean_names, means, stds, medians, mins, maxs
    ):
        import pandas as pd
        ...
        for i, (original_name, clean_name) in enumerate(zip(benchmarks, clean_names)):
            row = {
                ...
                "CV (%)": f"{(stds[i] / means[i] * 100):.2f}"
                if i < len(stds) and means[i] > 0
                else "N/A",
            }
        ...
        with open(txt_path, "w") as f:
            ...
            for col_name, width in col_widths.items():
                if col_name in ["Original Name", "Display Name"]:
                    header_line += f"{col_name:<{width}} "
            ...
            for _, row in df.iterrows():
                ...
                for col_name, width in col_widths.items():
                    ...
                    if (
                        col_name in ["Original Name", "Display Name"]
                        and len(value) > width - 1
                    ):
                        value = value[: width - 4] + "..."
            ...
            f.write(
                f"{'Fastest benchmark:':<25} {clean_names[fastest_idx]} ({means[fastest_idx]:.4f}s)\n"
            )
            f.write(
                f"{'Slowest benchmark:':<25} {clean_names[slowest_idx]} ({means[slowest_idx]:.4f}s)\n"
            )

            if len(means) > 1:
                speed_ratio = max(means) / min(means)
                f.write(f"{'Speed ratio:':<25} {speed_ratio:.2f}x (slowest/fastest)\n")
                ...
                f.write(
                    f"{'Average execution time:':<25} {mean_of_means:.4f}s ± {std_of_means:.4f}s\n"
                )
                f.write(
                    f"{'Coefficient of variation:':<25} {(std_of_means / mean_of_means) * 100:.2f}%\n"
                )
    ...
    def plot_resource_usage(self):
        ...
        clean_benchmarks, name_mapping = self._get_clean_benchmark_names(
            original_benchmarks
        )
        ...
        if (
            len(all_times) > 0
            and np.max(all_times) > 0
            and np.min(all_times[all_times > 0]) > 0
        ):
        ...
        bars = ax2.bar(
            ...
            memory_mb,
            alpha=0.8,
            color=sns.color_palette("viridis", len(memory_mb)),
        )
        ax2.set_xlabel("Benchmark", fontweight="bold")
        ...
        if (
            len(memory_mb) > 0
            and np.max(memory_mb) > 0
            and np.min(memory_mb[memory_mb > 0]) > 0
        ):
        ...
        ax3.legend(
            rating_handles,
            ["Poor (<25%)", "Fair (25-50%)", "Good (50-75%)", "Excellent (>75%)"],
            loc="upper right",
            frameon=True,
            fancybox=True,
        )
        ...
        if len(resource_data) > 1:
            metrics_df = pd.DataFrame(
                {
                    ...
                    "CPU Utilization (%)": (user_time + system_time)
                    / resource_data["elapsed_time"]
                    * 100,
                }
            )
            ...
            metrics_df["Time Score"] = (
                best_exec_time / metrics_df["Execution Time (s)"] * 100
            )
            metrics_df["Memory Score"] = (
                best_memory / metrics_df["Memory Usage (MB)"] * 100
            )
            ...
            heatmap_data = metrics_df.set_index("Benchmark")[
                ["Time Score", "Memory Score", "CPU Score", "Overall Score"]
            ]
            sns.heatmap(
                heatmap_data,
                annot=True,
            ...
            ax4.set_title(
                "Performance Scores (100 = best)", fontweight="bold", fontsize=14
            )
            ...
            ax4.text(
                0.5,
                -0.12,
                f"Best Overall: {best_bench} ({metrics_df.loc[best_idx, 'Overall Score']:.1f}/100)",
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "resource_usage.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    def plot_performance_counters(self):
        ...
        for i, metric in enumerate(metrics):
            ...
            if benchmark_names and values:
                clean_names, name_mapping = self._get_clean_benchmark_names(
                    benchmark_names
                )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "performance_counters.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    def plot_context_switches(self):
        ...
        for bench_name in self.context_stats_paths:
            ...
            if data is not None:
                ...
                context_switches.append(
                    self._get_perf_metric_value(data, "context-switches") or 0
                )
                cpu_migrations.append(
                    self._get_perf_metric_value(data, "cpu-migrations") or 0
                )
                page_faults.append(
                    self._get_perf_metric_value(data, "page-faults") or 0
                )
        ...
        if (
            context_switches
            and max(context_switches) > 0
            and min([cs for cs in context_switches if cs > 0]) > 0
        ):
        ...
        if (
            cpu_migrations
            and max(cpu_migrations) > 0
            and min([m for m in cpu_migrations if m > 0]) > 0
        ):
        ...
        if (
            page_faults
            and max(page_faults) > 0
            and min([pf for pf in page_faults if pf > 0]) > 0
        ):
        ...
        if any(tc > 0 for tc in task_clock):
            ...
            if (
                cs_per_sec
                and max(cs_per_sec) > 0
                and min([rate for rate in cs_per_sec if rate > 0]) > 0
            ):
            ...
            ax4.set_title(
                "Context Switch Rate (lower is generally better)",
                fontweight="bold",
                fontsize=14,
            )
            ...
            ax4.text(
                0.5,
                0.5,
                "No timing data for rate calculation",
                transform=ax4.transAxes,
                ha="center",
                va="center",
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "context_switches.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    def plot_scheduling_latency(self):
        ...
        clean_benchmark_names, name_mapping = self._get_clean_benchmark_names(
            benchmark_names
        )
        ...
        for i, bench_name in enumerate(self.sched_latency_paths):
            ...
            if (
                data is not None
                and not data.empty
                and "avg_delay_ms" in data.columns
                and "max_delay_ms" in data.columns
            ):
                ...
                sizes = (
                    20 + (sizes - sizes.min()) / (sizes.max() - sizes.min() + 0.1) * 200
                )

                cmap = (
                    plt.cm.viridis_r
                )  # Reversed so higher values (worse) are more visible
                norm = plt.Normalize(
                    data["max_delay_ms"].min(), data["max_delay_ms"].max()
                )
                ...
                ax.set_title(
                    f"{clean_name} - Scheduling Latency", fontweight="bold", fontsize=13
                )
                ...
                all_delays = np.concatenate(
                    [avg_delays[avg_delays > 0], max_delays[max_delays > 0]]
                )
                ...
                if len(all_delays) > 0:
                    ...
                    if ratio > 100:
                        ...
                        ax.set_yscale("log")
                        ax.set_xlabel(
                            "Average Delay (ms) - Log Scale", fontweight="bold"
                        )
                        ax.set_ylabel(
                            "Maximum Delay (ms) - Log Scale", fontweight="bold"
                        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "scheduling_latency.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    def _plot_latency_summary(self, summary_data):
        ...
        if (
            avg_latencies
            and max(avg_latencies) > 0
            and min([l for l in avg_latencies if l > 0]) > 0
        ):
        ...
        ax1.set_title(
            "Average Scheduling Latency Comparison", fontweight="bold", fontsize=14
        )
        ax1.set_xticklabels(ax1.get_xticklabels(), rotation=45, ha="right")
        ax1.grid(True, alpha=0.3)
        ...
        if use_log_avg:
            ax1.set_yscale("log")
            ax1.set_ylabel(
                "Average Scheduling Latency (ms) - Log Scale", fontweight="bold"
            )
        ...
        if (
            ...
            and max(worst_task_latencies) > 0
            and min([l for l in worst_task_latencies if l > 0]) > 0
        ):
        ...
        for i, (bar, val, task) in enumerate(
            zip(bars2, worst_task_latencies, worst_tasks)
        ):
            height = bar.get_height()
        ...
        if (
            total_switches
            and max(total_switches) > 0
            and min([s for s in total_switches if s > 0]) > 0
        ):
        ...
        if len(clean_names) > 2:
            try:
                ...
                slope, intercept, r_value, p_value, std_err = stats.linregress(
                    ...
                    np.log10(worst_task_latencies)
                    if use_log_worst
                    else worst_task_latencies,
                )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "latency_summary.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    def _plot_task_latency_distribution(self):
        ...
        for i, switches_val in enumerate(switches):
            ax1.text(
                i,
                0.1 if not use_log_task else min(all_delays) * 0.1,
                f"{switches_val:,} sw",
                ha="center",
                va="bottom",
                fontsize=9,
        ...
        task_benchmark_matrix = pd.crosstab(
            latency_df["task"],
            latency_df["clean_benchmark"],
            values=latency_df["max_delay"],
            aggfunc="max",
        ).fillna(0)
        ...
        if not task_benchmark_matrix.empty and task_benchmark_matrix.shape[0] > 1:
            ...
            task_labels = [
                t[:25] + "..." if len(t) > 25 else t for t in plot_matrix.index
            ]
            ...
            ax2.text(
                0.5,
                0.5,
                "Insufficient data for task/benchmark heatmap",
                transform=ax2.transAxes,
                ha="center",
                va="center",
                fontsize=12,
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "task_latency_distribution.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig)
    ...
    def plot_correlation_analysis(self):
        ...
        benchmarks = set(
            list(self.hyperfine_paths.keys())
        ...
        for bench_name in benchmarks:
            ...
            if resource_data is not None:
                resource_row = resource_data[resource_data["benchmark"] == bench_name]
            ...
            if bench_name in self.perf_counter_paths:
                ...
                if perf_data is not None:
                    ...
                    row["instructions"] = self._get_perf_metric_value(
                        perf_data, "instructions"
                    )
                    row["cache_references"] = self._get_perf_metric_value(
                        perf_data, "cache-references"
                    )
                    row["cache_misses"] = self._get_perf_metric_value(
                        perf_data, "cache-misses"
                    )
                    ...
                    if row.get("cache_references") and row.get("cache_misses"):
                        row["cache_miss_rate"] = (
                            row["cache_misses"] / row["cache_references"] * 100
                        )
            ...
            if bench_name in self.context_stats_paths:
                ...
                if context_data is not None:
                    row["context_switches"] = self._get_perf_metric_value(
                        context_data, "context-switches"
                    )
                    row["cpu_migrations"] = self._get_perf_metric_value(
                        context_data, "cpu-migrations"
                    )
                    del context_data
        ...
        if len(correlation_data) > 1:
            ...
            if len(numeric_cols) > 1:
                ...
                corr_df = corr_df.rename(
                    columns={col: pretty_names.get(col, col) for col in corr_df.columns}
                )
                ...
                if corr_df.shape[1] > 1:
                    ...
                    try:
                        ...
                        plt.tight_layout()
                        plt.savefig(
                            self.output_dir / "correlation_analysis.png",
                            dpi=300,
                            bbox_inches="tight",
                        )
                        plt.close(fig)
    ...
    def _generate_scatter_matrix(self, df, pretty_names):
        ...
        if len(key_metrics) >= 3:
            ...
            plot_df = plot_df.rename(
                columns={col: pretty_names.get(col, col) for col in key_metrics}
            )
            ...
            try:
                ...
                scatter_matrix.fig.suptitle(
                    "Performance Metrics Relationships", fontsize=16, y=1.02
                )

                plt.tight_layout()
                plt.savefig(
                    self.output_dir / "metrics_scatter_matrix.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
    ...
    def plot_sched_timeline(self):
        ...
        for i, bench_name in enumerate(self.sched_timeline_paths):
            ...
            if estimated_total > 50000:
                ...
                print(
                    f"  Large timeline dataset detected ({estimated_total:,} est. points), using {sample_rate:.1%} sample"
                )
            del sample_data

            timeline_data = self._get_sched_timeline(
                bench_name, sample_rate=sample_rate
            )
            if timeline_data is None or timeline_data.empty:
                continue

            print(
                f"  Generating visualization with {len(timeline_data):,} data points..."
            )
            ...
            task_types = {
                "idle": [
                    t
                    for t in timeline_data["task_name"].unique()
            ...
            if time_segments:
                timeline_collection = LineCollection(
                    time_segments, colors=colors_list, linewidths=7, alpha=0.8
                )
            ...
            ax_overview.set_title(
                f"Scheduling Timeline Overview - {clean_name}",
                fontweight="bold",
                fontsize=14,
                pad=10,
            )
            ...
            top_tasks = (
                task_stats.sort_values("importance", ascending=False)
                .head(15)
                .index.tolist()
            )

            if not any(t in task_types["idle"] for t in top_tasks):
                idle_tasks = task_stats[
                    task_stats.index.isin(task_types["idle"])
                ].sort_values("run_time_ms_sum", ascending=False)
                if not idle_tasks.empty:
                    top_tasks.append(idle_tasks.index[0])

            if not any(t in task_types["system"] for t in top_tasks):
                system_tasks = task_stats[
                    task_stats.index.isin(task_types["system"])
                ].sort_values("run_time_ms_sum", ascending=False)
            ...
            for cpu in cpus:
                cpu_data = timeline_data[timeline_data["cpu"] == cpu].sort_values(
                    "timestamp"
                )
            ...
            for task in top_tasks:
                ...
                legend_handles.append(
                    Patch(color=top_task_colors[task], label=short_name)
                )
            ...
            if len(non_zero_delays) > 0:
                ...
                if use_log:
                    ...
                    ax_delays.set_xlabel(
                        "Scheduling Delay (ms) - Log Scale", fontweight="bold"
                    )
                else:
                    bins = 30
                    ax_delays.hist(
                ...
                for pct, val, color in zip(
                    percentiles, percentile_values, percentile_colors
                ):
                    ax_delays.axvline(
                        val,
                        color=color,
                ...
                ax_delays.set_title(
                    "Scheduling Delay Distribution", fontweight="bold", fontsize=14
                )
                ...
                ax_delays.text(
                    0.5,
                    0.5,
                    "No non-zero scheduling delays found",
                    transform=ax_delays.transAxes,
                    ha="center",
                    va="center",
                )
            ...
            time_bins = np.linspace(
                0, timeline_duration_ms, min(100, int(timeline_duration_ms / 10) + 1)
            )
            ...
            ax_util.set_title(
                "CPU Utilization Over Time (%)", fontweight="bold", fontsize=14
            )
            ...
            plt.tight_layout()
            safe_name = (
                clean_name.replace(" ", "_").replace("/", "_").replace("\\", "_")
            )
            plt.savefig(
                self.output_dir / f"{safe_name}_sched_timeline.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close(fig)
            ...
            self._plot_task_timeline(
                bench_name, clean_name, timeline_data, top_tasks, top_task_colors
            )
    ...
    @_with_memory_limit
    def _plot_task_timeline(
        self, bench_name, clean_name, timeline_data, top_tasks, task_colors
    ):
        if timeline_data is None or timeline_data.empty:
            return
        ...
        task_durations = (
            timeline_data.groupby("task_name")["run_time_ms"]
            .sum()
            .sort_values(ascending=False)
        )

        significant_tasks = task_durations[
            task_durations > task_durations.sum() * 0.01
        ].index.tolist()
        ...
        idle_tasks = [
            ...
            if any(
                x in t.lower()
                for x in ["idle", "kworker", "system", "swapper", "migration"]
            )
        ]
        display_tasks = important_tasks[:20]

        task_first_seen = {
            task: timeline_data[timeline_data["task_name"] == task]["timestamp"].min()
            for task in display_tasks
        }
        display_tasks = sorted(
            [t for t in display_tasks if t not in idle_tasks],
            key=lambda x: task_first_seen[x],
        ) + sorted(idle_tasks)

        fig, (ax1, ax2) = plt.subplots(
            2, 1, figsize=(16, 12), gridspec_kw={"height_ratios": [3, 1], "hspace": 0.3}
        )
        ...
        for _, row in sorted_data.iterrows():
            ...
            if (
                task in display_tasks and run_time > 0.1
            ):  # Only include meaningful durations
                segments[task].append((time_ms, time_ms + run_time, cpu))
                displayed_runtime += run_time

        hidden_runtime_pct = (
            100 - (displayed_runtime / total_runtime * 100) if total_runtime > 0 else 0
        )
        ...
        for task in idle_tasks:
            if task in task_cpu_times:
                for cpu in cpus:
                    idle_cpu_util[cpu] += (
                        task_cpu_times[task][cpu] / timeline_duration_ms * 100
        ...
        for task_idx, task in enumerate(display_tasks):
            ...
            for start, end, cpu in task_segments:
                verts.append(
                    [(start, y - 0.4), (start, y + 0.4), (end, y + 0.4), (end, y - 0.4)]
                )
        ...
        for task in display_tasks:
            cpu_percentages = [
                (task_cpu_times[task][cpu] / timeline_duration_ms * 100) for cpu in cpus
            ]
        ...
        cpu_patches = [
            Patch(color=cpu_colors[cpu % len(cpu_colors)], label=f"CPU {cpu}")
            for cpu in cpus
        ]
        ...
        ax1.legend(
            handles=cpu_patches, loc="upper right", ncol=2, fancybox=True, shadow=True
        )

        time_bins = np.linspace(
            0, timeline_duration_ms, min(200, int(timeline_duration_ms / 5) + 1)
        )
        ...
        for _, row in timeline_data.iterrows():
            ...
            for b in range(start_bin, end_bin):
                ...
                if row["task_name"] in display_tasks and duration > 0.1:
                    shown_activity[cpu_idx, b] += duration / (
                        time_bins[b + 1] - time_bins[b]
                    )

        hidden_mask = (all_activity > 0.1) & (shown_activity < 0.05)

        all_activity = np.clip(
            all_activity / all_activity.max()
            if all_activity.max() > 0
            else all_activity,
            0,
            1,
        )
        ...
        ax2.set_title(
            "CPU Activity Intensity (red outline = activity not shown in timeline)",
            fontweight="bold",
            fontsize=14,
        )
        ...
        missing_pct = (
            ((all_activity > 0.1) & (shown_activity < 0.05)).sum()
            / (all_activity > 0).sum()
            * 100
        )
        if missing_pct > 10:
        ...
        plt.savefig(
            self.output_dir / f"{safe_name}_task_timeline.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig)
    ...
    def plot_ridgeline(self):
        try:
            ...
            if len(data_dict) > 1:
                ...
                plt.savefig(
                    self.output_dir / "ridgeline_plot.png", dpi=300, bbox_inches="tight"
                )
                plt.close()
            ...
            print(
                "joypy package not available for ridgeline plots. Install with: pip install joypy"
            )
        except Exception as e:
    ...
    def generate_summary_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            try:
                ...
                if cpu_freq:
                    f.write(
                        f"CPU Frequency: {cpu_freq.current:.2f} MHz (min: {cpu_freq.min:.2f}, max: {cpu_freq.max:.2f})\n"
                    )
                f.write(
                    f"CPU Count: {psutil.cpu_count(logical=True)} logical, {psutil.cpu_count(logical=False) or '?'} physical\n"
                )
                ...
                f.write(
                    "System information not available (psutil package not installed)\n\n"
                )
            except Exception as e:
                if self.debug_mode:
            ...
            if self.hyperfine_paths:
                ...
                f.write(
                    f"{'Benchmark':<30} {'Mean (s)':<10} {'Std Dev':<10} {'Min (s)':<10} {'Max (s)':<10} {'vs Fastest':<10}\n"
                )
                f.write("-" * 80 + "\n")

                for data in execution_data:
                    vs_fastest = (
                        data["mean"] / fastest_time if fastest_time > 0 else 1.0
                    )
                    vs_fastest_str = (
                        f"{vs_fastest:.2f}x"
                        if data["clean_name"] != fastest_bench
                        else "1.00x (fastest)"
                    )

                    f.write(
                        f"{data['clean_name'][:30]:<30} {data['mean']:.4f}     {data['std']:.4f}     "
            ...
            if resource_data is not None and not resource_data.empty:
                ...
                f.write(
                    f"{'Benchmark':<30} {'Memory (MB)':<12} {'CPU Time (s)':<12} {'User Time (s)':<12} {'Sys Time (s)':<12}\n"
                )
                f.write("-" * 80 + "\n")
                ...
                for _, row in resource_data.iterrows():
                    ...
                    f.write(
                        f"{clean_name[:30]:<30} {memory_mb:>11.1f} {cpu_time:>11.2f} {user_time:>11.2f} {sys_time:>11.2f}\n"
                    )

                if len(resource_data) > 1:
                    f.write("-" * 80 + "\n")
                    f.write(
                        f"{'AVERAGE':<30} {resource_data['max_rss_kb'].mean() / 1024:>11.1f} "
            ...
            if self.perf_counter_paths:
                ...
                for bench_name in self.perf_counter_paths:
                    ...
                    if data is not None:
                        ...
                        ipc = (
                            values.get("instructions", 0) / values.get("cycles", 1)
                            if values.get("cycles", 0) > 0
                            else 0
                        )
                        miss_rate = (
                            values.get("cache-misses", 0)
                            / values.get("cache-references", 1)
                            * 100
                            if values.get("cache-references", 0) > 0
                            else 0
                        )
            ...
            if self.context_stats_paths or self.sched_latency_paths:
                ...
                if self.context_stats_paths:
                    f.write(
                        f"{'Benchmark':<30} {'Context Switches':<20} {'CPU Migrations':<20} {'Page Faults':<15}\n"
                    )
                    f.write("-" * 85 + "\n")
                    ...
                    for bench_name in self.context_stats_paths:
                        ...
                        if data is not None:
                            cs_count = self._get_perf_metric_value(
                                data, "context-switches"
                            )
                            mig_count = self._get_perf_metric_value(
                                data, "cpu-migrations"
                            )
                            pf_count = self._get_perf_metric_value(data, "page-faults")

                            cs_str = (
                                f"{cs_count:,.0f}" if cs_count is not None else "N/A"
                            )
                            mig_str = (
                                f"{mig_count:,.0f}" if mig_count is not None else "N/A"
                            )
                            pf_str = (
                                f"{pf_count:,.0f}" if pf_count is not None else "N/A"
                            )

                            f.write(
                                f"{clean_name[:30]:<30} {cs_str:<20} {mig_str:<20} {pf_str:<15}\n"
                            )
                            del data
                ...
                if self.sched_latency_paths:
                    ...
                    f.write(
                        f"{'Benchmark':<30} {'Avg Latency (ms)':<18} {'Max Latency (ms)':<18} {'Worst Task':<30}\n"
                    )
                    f.write("-" * 96 + "\n")
                    ...
                    for bench_name in self.sched_latency_paths:
                        ...
                        if data is not None and not data.empty:
                            ...
                            f.write(
                                f"{clean_name[:30]:<30} {avg_latency:>17.4f} {max_latency:>17.4f} {worst_task[:30]}\n"
                            )
                            del data
...
Examples:
  ...
  python benchmark_visualizer.py session_20250812_100137 --plots execution,latency
    ...
    parser.add_argument("session_dir", help="Path to the benchmark session directory")
    ...
    try:
        start_time = time.time()
        print(
            f"Starting benchmark visualization at {time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
            ...
            for i, plot_type in enumerate(plot_types):
                if plot_type in plot_functions:
                    ...
                    try:
                        ...
                        print(
                            f"  ✓ Completed in {time.time() - plot_start:.1f} seconds"
                        )
                    except Exception as e:
